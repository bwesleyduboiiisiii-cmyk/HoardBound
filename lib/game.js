...
export function directorEffect(kind, room, playersIn) {
  ...
  if (kind === "meteor") {
    ...
    const poor = P.slice().sort((a, b) => a.gold - b.gold)[0];
    if (poor) { const g = 1800; poor.gold += g; // viewer gift: bonus gold, NOT taken from the hoard
...
export function feedClass(kind) {
  ...
  if (kind === "pact" || kind === "pact_offer") return "pact";
...
export function giftEffect(giftType, room, players, opts = {}) {
  ...
  if (giftType === "rose") {
    ...
    else if (r.id === "dsneak") { const t = pick(eligiblePlayers(P)); modifiers.doubleSneak = { ...(modifiers.doubleSneak || {}), [t.id]: true }; mark(t); effect = "Silent Fortune"; line = `${t.name}'s next Sneak pays double.`; }
