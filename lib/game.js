...
// --- Narration timeline (shared by host + overlay) -------------------------
// The host composes the whole narration once and writes it to rooms.narration
// with a startedAt anchor. Both screens then derive the current line purely
// from the clock using these two helpers, so they stay in perfect lockstep
// regardless of realtime latency — only the first write has to propagate.

// Deterministic on-screen time for one line (ms): reading/speaking budget.
export function narrationDur(text) {
  const len = (text || "").length;
  return Math.max(2400, Math.min(7000, Math.round(len * 70) + 900));
}

// Current line index for a narration payload at clock time `now` (ms epoch).
// Returns lines.length once the whole sequence has played out.
export function narrationIndexAt(nar, now) {
  if (!nar || !nar.lines || !nar.lines.length || nar.startedAt == null) return 0;
  const durs = nar.durs && nar.durs.length === nar.lines.length
    ? nar.durs : nar.lines.map((l) => narrationDur(l.text));
  let acc = new Date(nar.startedAt).getTime();
  for (let i = 0; i < nar.lines.length; i++) {
    acc += durs[i];
    if (now < acc) return i;
  }
  return nar.lines.length;
}
...
// The real TikTok gift that triggers each power (used by the /gifts guide overlay). Remap freely.
export const GIFT_TIKTOK = {
  treasure_box: "Treasure Box", lightning_bolt: "Lightning Bolt", shard_hope: "A Shard of Hope",
  dragons_breath: "Chili", claw_swipe: "Spinning Soccer", tail_lash: "Gold Boxing Gloves",
  wing_gust: "Rosa", ember_storm: "Smores", fireball: "Confetti", scorch_earth: "Panther Paws",
  pirates_treasure: "Pirate's Treasure", money_gun: "Money Gun", warrior_shell: "Shell of a Warrior",
  sound_spell: "Sound Spell", gold_mine: "Gold Mine", lovers_lock: "Lover's Lock",
  dragon_bite: "Baby Dragon", meteor_storm: "Meteor Shower", dragon_flame: "Dragon Flame",
  tiktok_universe: "TikTok Universe",
};
export const GIFT_ATTACKS = new Set([
  "dragons_breath", "claw_swipe", "tail_lash", "wing_gust", "ember_storm",
  "fireball", "scorch_earth", "dragon_bite", "meteor_storm", "dragon_flame",
]);
...
export function giftEffect(giftType, room, players, opts = {}) {
  ...
  if (giftType === "treasure_box") {
    ...
    rage = 0;
    effect = "Dragon Flame";
    line = `${sender} unleashed DRAGON FLAME — the dragon torches everything, EVERY hunter burned down to 0 ◈!`;
