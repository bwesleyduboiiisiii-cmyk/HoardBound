...
// Host mirrors the current narration line to the room so the LIVE overlay can show it too.
export async function setRoomNarration(roomId, line) {
  try { await supabase.from("rooms").update({ narration: line || null }).eq("id", roomId); } catch (e) {}
}

// ---- Live TikTok chat (bridge writes, overlay reads) ----
export async function addChat(roomId, name, text) {
  try { await supabase.from("chats").insert({ room_id: roomId, name: (name || "viewer").slice(0, 24), text: (text || "").slice(0, 160) }); } catch (e) {}
}
export async function getChats(roomId, limit = 30) {
  const { data } = await supabase.from("chats").select("id,name,text").eq("room_id", roomId).order("id", { ascending: false }).limit(limit);
  return (data || []).reverse();
}
