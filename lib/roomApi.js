...
export async function signInAccount(username, code, avatarUrl) {
  ...
  if (existing) {
    if (existing.code !== pin) throw new Error("That username is taken, or the code is wrong.");
    // returning user — update their photo if they changed it
  ...
  await Promise.all(
    eff.players
      .filter((np) => {
        ...
        return old && (old.gold !== np.gold || old.trust !== np.trust || old.warded !== np.warded);
      })
      .map((np) => supabase.from("players").update({ gold: np.gold, trust: np.trust, warded: np.warded }).eq("id", np.id))
  );
  await supabase.from("rooms").update({
    rage: eff.room.rage, hoard: eff.room.hoard, double_next: eff.room.double_next,
  }).eq("id", room.id);
...
export async function endGame(roomId) {
  ...
  if (error) throw error;
  await recordResults(players.map((p) => ({ name: p.name, gold: p.gold })));
}
...
export async function recordResults(results) {
  try {
    ...
    const { data: existing } = await supabase.from("leaders").select("*").in("name", names);
    ...
    const rows = results.map((r) => {
      ...
      return {
        name: r.name,
        games: e.games + 1,
        wins: e.wins + (winner && r.name === winner.name ? 1 : 0),
        total_gold: Number(e.total_gold) + Number(r.gold || 0),
        best_gold: Math.max(Number(e.best_gold), Number(r.gold || 0)),
        updated_at: new Date().toISOString(),
      };
    });
    await supabase.from("leaders").upsert(rows, { onConflict: "name" });
  } catch (e) { /* leaders table may not be migrated yet — never break the game */ }
}
