...
export async function resolveRound(room) {
  ...
  // clear one-round gift modifiers; jsonb column may not be migrated yet, so never let this break resolve
  try { await supabase.from("rooms").update({ modifiers: {} }).eq("id", room.id); } catch (e) {}
